...
bool ok(int q[], int col) {
    ...
    for (int i = 0; i < col; i++) {
        ...
        if (mp[newMan][q[newMan]] < mp[newMan][currentWoman]){
            if (wp[currentWoman][q[currentWoman]] > wp[currentWoman][newMan]) {
                return false;
            }
        }
        /*
        if (wp[newWoman][q[newWoman]] > wp[newWoman][currentMan]) {
            if (mp[currentMan][q[currentMan]] > mp[currentMan][newWoman]) {
                cout << "check2: ";
                return false;
            }
        }
